...
class Groups {
    ...
    members (options, { offset = 0, count = 0, sort = undefined, fields = undefined, query = undefined} = {}, callback) {
        Object.assign(options, arguments[1]);
        return this.client.request("GET", "groups.members", options, callback);
    }
    ...
    setCustomFields({roomId, roomName} = {}, customFields = {}, callback) {
      if (roomId)
        ...
        return this.client.request("POST", "groups.setCustomFields", {roomName, customFields}, callback);
      else {
        let errMsg = "roomId or roomName is required";
        callback(errMsg);
        return Promise.reject(errMsg);
